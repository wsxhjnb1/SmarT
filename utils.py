...
def load_unlabel_data(args):
    if args.dataset in ['last_letters', 'coin_flip', 'StrategyQA', 'CommonsenseQA', 'AQuA', 'gsm8k', "SVAMP"]:
        with open('./dataset/{}/{}_corpus.json'.format(args.dataset,args.dataset),'r') as f:
            sample = json.load(f)
    else:
        with open('./dataset/SVAMP/SVAMP_corpus.json','r') as f:
            sample = json.load(f)
    return sample
...
def load_data(args):
    ...
    answers = []
    ids = []
    datapath = '{}/{}/{}.json'.format(Dataset_Folder, args.dataset, args.dataset)
