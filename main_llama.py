...
from llama_runner import llama_runner_chat
...
llm_type = args.engine
gpu_memory_utilization = 0.95

#set path here
llm = LLM(model=f"{llm_type}", gpu_memory_utilization=gpu_memory_utilization, tensor_parallel_size=2)
...
def LLMs_reasoning():
    ...
    if os.path.exists("demos/{}_{}_test_set_diff.json".format(args.engine,args.dataset.lower())):
        ...
        zero_preds = llama_runner_chat(llm, args, inputs_all, 0, args.max_length_cot, n=1)
        zsc["cot"] = zero_preds
        for idx, cot_answer in enumerate(zero_preds):
            ...
            input_clean = text_cleanup(question[idx], stopwords)
            sum = 0
    ...
    if os.path.exists("demos/{}_{}_demo_set_diff_{}.json".format(args.engine,dataset_,args.sample_question_num)):
        ...
        if args.dataset.lower() == "aqua":
            sample_question_temp = random.sample(unlabel_datas, args.sample_question_num)
            sample_question = []
            for s_q in sample_question_temp:
                if len(s_q.split()) <= 60:
                    sample_question.append(s_q)
        else:
        ...
        corpus_preds = llama_runner_chat(llm, args, inputs_all, 0, args.max_length_cot, n=1)
        zsc_corpus['cot'] = corpus_preds
        for idx, cot_answer in enumerate(corpus_preds):
            zsc_corpus["syntax_complex"].append(len(cot_answer.split()))
            output_clean = text_cleanup(cot_answer, stopwords)
            output_clean = set(output_clean)
            input_clean = text_cleanup(sample_question[idx], stopwords)
            sum = 0
            for token in output_clean:
                if token not in input_clean:
                    sum += 1
    ...
    if os.path.exists("demos/{}_{}_fsc_{}.json".format(args.engine,dataset_,args.sample_question_num)):
        ...
        for idx, element in enumerate(zsc_corpus_question):
            ...
            if corpus_diff[idx] == 'hard':
                ...
                inputs = inputs_easy
            inputs_all.append(inputs)
        fsc_preds = llama_runner_chat(llm, args, inputs_all, 0, args.max_length_cot, n=1)
        for idx, cot_answer in enumerate(fsc_preds):
    ...
    for idx, element in enumerate(question_pred):
        ...
        inputs_all.append(inputs)
    adot_preds = llama_runner_chat(llm, args, inputs_all, 0, args.max_length_cot, n=1)
    for idx, pred in enumerate(adot_preds):
        pred_answer = None
        pred_for_save = None
        flag = 0
        repeat_times = 0
        while pred_answer == None:
            if repeat_times != 0:
                pred = llama_runner_chat(llm, args, inputs_all[idx], flag, args.max_length_cot, n=1)[0]
            repeat_times += 1
            if repeat_times > 3:
                break
            pred_for_save = pred
            if 'answer is' in pred or 'answer choice is' in pred:
                ...
                inputs2 = inputs_all[idx] + pred + ' ' + args.direct_answer_trigger_for_direct
                try:
                    pred3 = llama_runner_chat(llm, args, inputs2, 0, 32)
                    print('no answer:')
                    print(pred3)
